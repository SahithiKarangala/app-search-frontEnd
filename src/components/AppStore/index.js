...
class AppStore extends Component {
  ...
  onSearchApp = event => {
    ...
    const searchedApps = appsList.filter(eachItem =>
      eachItem.appName.toLowerCase().includes(name.toLowerCase()),
    )
    this.setState({appsListContainer: searchedApps})
  }

  render() {
    const {categoryId, appsListContainer} = this.state
    const filteredAppList = appsListContainer.filter(
      appItem => appItem.category === categoryId,
    )
